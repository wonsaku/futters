...
export default async function MatchDetailPage({
  ...
  const author = match.author as { nickname: string } | null

  return (
    <main className="max-w-3xl mx-auto px-4 py-8">
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          <Link href="/matches" className="text-sm mb-3 inline-block" style={{ color: 'var(--futters-gray)' }}>
            ← 목록으로
          </Link>
          <h1 className="text-2xl font-black" style={{ color: 'var(--futters-dark)' }}>
            vs {match.opponent}
          </h1>
      ...
      {match.notes && (
        <div className="futters-card p-5">
          <p className="text-xs font-semibold mb-2" style={{ color: 'var(--futters-gray)' }}>메모</p>
      ...
      <p className="text-xs mt-4 text-center" style={{ color: 'var(--futters-gray)' }}>
        기록자: {author?.nickname}
      </p>
    </main>
  )
